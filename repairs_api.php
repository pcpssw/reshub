...
if ($action === 'list_user_history') {
    // ตรวจสอบ user_id จากทั้ง $_POST และ req_param เพื่อความแม่นยำ
    $userId = isset($_POST['user_id']) ? intval($_POST['user_id']) : intval(req_param('user_id', 0));

    if ($userId <= 0) {
        repair_json(['success' => false, 'ok' => false, 'message' => 'UserId missing'], 400);
    }

    // ดึงรายการแจ้งซ่อมทั้งหมดที่ผูกกับ user_id นี้
    $sql = "SELECT r.*, rt.type_name, rm.room_number 
            FROM rh_repairs r
            LEFT JOIN rh_repair_types rt ON rt.type_id = r.type_id
            LEFT JOIN rh_rooms rm ON rm.room_id = r.room_id
            WHERE r.user_id = ? 
